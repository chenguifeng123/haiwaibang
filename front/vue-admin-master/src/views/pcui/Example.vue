<template>
  <section>
    ...
    <el-table
      ...
      style="width: 100%;"
    >
      <el-table-column prop="id" label="案例编号" v-if="false" width="150" v-show="false" sortable></el-table-column>
...
export default {
  ...
  methods: {
    getTransferObject (object) {
      return util.fillPath(object, 'example');
    },
    ...
    //获取用户列表
    getExamples () {
      util.getRightTableData(this, { dataSetName: 'examples' });
    },
    ...
    //显示新增界面
    handleAdd: function () {
      this.$router.push({ path: '/content', query: { type: 'example' } });
