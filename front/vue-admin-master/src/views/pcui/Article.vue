<template>
  <section>
    ...
    <el-table
      ...
      style="width: 100%;"
    >
      <el-table-column prop="id" label="文章编号" v-if="false" width="150" v-show="false" sortable></el-table-column>
...
export default {
  ...
  methods: {
    getTransferObject (object) {
      return util.fillPath(object, 'article');
    },
    ...
    //获取用户列表
    getArticles () {
      util.getRightTableData(this, { dataSetName: 'articles' });
    },
    ...
    //显示新增界面
    handleAdd: function () {
      this.$router.push({ path: '/content', query: { type: 'article' } });
